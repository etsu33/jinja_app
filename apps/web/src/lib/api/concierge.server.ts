...
import type { ConciergeThread, ConciergeThreadDetail } from "./concierge/types";
...
export async function getConciergeThreadsServer(): Promise<ConciergeThread[]> {
  const h = await headers();
  const baseUrl = resolveServerBaseUrlFromHeaders(h);
  const cookieHeader = h.get("cookie") ?? "";

  const res = await fetch(`${baseUrl}/api/concierge-threads/`, {
    method: "GET",
    cache: "no-store",
    headers: {
      Accept: "application/json",
  ...
  if (!res.ok) {
    throw new Error(`getConciergeThreadsServer failed: ${res.status}`);
  }

  const data = await res.json();

  if (Array.isArray(data)) return data as ConciergeThread[];
  if (Array.isArray(data?.results)) return data.results as ConciergeThread[];
