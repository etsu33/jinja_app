...
function getItemClassName(item: DetailMeaningItem, variant: ShrineDetailSectionVariant): string {
  // Borderless editorial flow (PR-G3): drop the per-item sub-cards entirely.
...
function getTitleClassName(item: DetailMeaningItem, variant: ShrineDetailSectionVariant): string {
  // Supplementary keys stay visually quieter than core meaning in both variants.
  if (item.key === "history_context" || item.key === "deity_symbol" || item.key === "benefit_action") {
    return "text-xs font-semibold tracking-[0.04em] text-[var(--kt-color-text-muted)]";
  }

  if (variant === "plain") {
    return "text-sm font-semibold text-[var(--kt-color-text-primary)]";
  }
...
function getBodyClassName(item: DetailMeaningItem, variant: ShrineDetailSectionVariant): string {
  if (item.key === "history_context" || item.key === "deity_symbol" || item.key === "benefit_action") {
    return "mt-1 text-xs leading-6 text-[var(--kt-color-text-muted)]";
  }

  if (variant === "plain") {
    return "mt-1 text-[15px] leading-7 text-[var(--kt-color-text-secondary)]";
  }
...
function MeaningItems({ items, variant }: { items: DetailMeaningItem[]; variant: ShrineDetailSectionVariant }) {
  return (
    <div className={variant === "plain" ? "space-y-4" : "space-y-3"}>
      {items.map((item) => (
        <div key={item.key} className={getItemClassName(item, variant)}>
          <h3 className={getTitleClassName(item, variant)}>{item.title}</h3>
          <p className={getBodyClassName(item, variant)}>{item.body}</p>
        </div>
      ))}
    </div>
  );
...
export default function ShrineJudgeSection({ section, variant = "card" }: Props) {
  if (isSupplementSection(section)) {
    // Supplementary background ("補足：…") is Evidence -> stays a <details>
    // disclosure regardless of variant (PR-G3 Phase 7).
    return (
      <details className="rounded-[var(--kt-radius-card)] border border-[var(--kt-color-border-default)] bg-[var(--kt-color-surface-default)] p-4">
        ...
        <div className="mt-4">
          <MeaningItems items={section.items} variant="card" />
        </div>
      </details>
    );
  }
  ...
  return (
    <section className={variant === "plain" ? "" : SHRINE_DETAIL_SECTION_CARD_CLASS}>
      ...
      <div className="mt-4">
        <MeaningItems items={section.items} variant={variant} />
      </div>
    </section>
  );
}
