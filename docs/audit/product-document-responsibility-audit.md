...
| 分類 | 該当行数 | 内容 |
|---|---:|---|
| 現行実装が生成する値と完全一致 | 2件 | 「不安が強い」「疲れている」 |
| 状態検知は反応するが、宣言された主/補助テーマの組み合わせが実際の出力と一致しない | 4件 | 「やり直したい」「人間関係で悩んでいる」「健康が不安」「お金が不安」 |
| 状態検知ロジックが一切反応しない（未実装の意味的例示） | 10件 | 「将来が見えない」「落ち込んでいる」「転職を考えている」「独立したい」「挑戦したい」「決断したい」「自信がない」「勉強したい」「商売を伸ばしたい」「自分を見つめ直したい」 |

「完全一致」以外の14件は、Product文書が期待する意味的対応と、現行の相談解釈実装が実際に生成する値が異なる（またはそもそも生成されない）。特に「落ち込んでいる」「自信がない」の2件は、修正前は「復興」を主テーマとする例として記載されていたが、「復興」は現行実装では主テーマとして生成されえない（15節参照）ため、二重に不正確だった。
