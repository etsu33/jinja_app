...
- Design authority: `docs/core/split-anchor-architecture.md`（PHASE_1、本書では変更しない）
- Base: `origin/develop` = `31cf9dd21db84c77f0ba3715915fe471f0ab50e5`
...
`anchor`（FK, CASCADE, `related_name="evidences"`）/ `component`（nullable FK, CASCADE）/
...
```text
backend full suite                         : 3965 passed, 12 skipped  (実装後)
relevant migration tests                   : 129 passed
...
rollback / reapply (standard)               : 0115 → 0114 → 0115 OK
...
- `docs/core/split-anchor-architecture.md`（header の `Schema implementation: NOT_STARTED` を含め未変更。更新要否は Mother Ship 判断）
...
- `CONFIRMED + UNWEIGHTED_COMPONENT_MEAN` で `subject_type != MULTI_PRINCIPAL_UNIT` の組合せは、指示・正本とも規定がないため制約していない（mean一致検証の対象外）。
- Evidence の `source_type` 等の正式taxonomy、および Evidence の必須field要件は PHASE_2 で定義する。
- Evidence.component の `on_delete` は CASCADE とした（component 削除で紐付く Evidence も消える）。監査保全上 PROTECT / RESTRICT が望ましいかは未決定。
