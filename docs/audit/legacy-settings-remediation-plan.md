...
| 24 | ~~`apps/web`の期限超過互換ルート（`/api/shrines/[id]`）の削除~~ **対応済み** | 削除 | **P0** | 実施（本番アクセスログはVercel Hobbyプランの保持期間1時間のため長期実測は不可能と判明。正本ルートへの完全代替とコード参照0件をもって削除を判断） |
...
1. **P0（#15, #24, #25）**: 誤解・期限超過リスクがあるため最優先。#15・#24は対応済み
