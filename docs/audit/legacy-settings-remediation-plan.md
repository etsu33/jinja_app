...
| 24 | ~~`apps/web`の期限超過互換ルート（`/api/shrines/[id]`）の削除~~ **対応済み**（PR #2071） | 削除 | **P0** | 必要（本番アクセスログ、削除実行の判断材料として）→ 実測不可能と判明したため構造的代替とコード参照0件を根拠に削除済み |
...
**P0（3件）の選定理由**: いずれも「現状のまま放置すると誤解・誤操作につながる」項目である。#15は命名が「shadow専用」を示唆するが実際には本番ランキングへ直接影響しており、この命名を信じた開発者が誤って無効化・変更するリスクがある。#24は文書化された削除期限（2026-04-01）を本監査時点で3.5ヶ月超過しており、削除保留の判断が更新されないまま放置されていた。#25は「デモ用の一時対応」が本番コードに固定化されたままであり、コード上のコメントが示す意図（デモ後に環境変数制御へ戻す）と実態が乖離している。**P0の3件は現時点で全て対応済みである**（#15: リネーム済み、#24: PR #2071で削除済み、#25: 新レンダラーへ一本化済み）。
...
1. **P0（#15, #24, #25）**: 誤解・期限超過リスクがあるため最優先。**3件とも対応済み**（#24はアクセスログ実測が技術的に不可能と判明したため、構造的代替とコード参照0件を根拠にPR #2071で削除済み）
