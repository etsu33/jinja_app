...
>
> **Documentation reconciliation (P10, docs-only)**: §8, §13 and §19 were updated to current mapping truth. `travel_safe = {3, 13, 14}` (交通安全 / 航海安全 / 海上安全) in the runtime module `backend/temples/domain/need_to_goriyaku_tag_ids.py` (pinned by `backend/temples/tests/test_need_to_goriyaku_tag_ids.py`); the earlier draft's claim that ids 13/14 were unwired, and its reference to stale ids 42–45, were stale. See `docs/audit/recommendation-evidence-contract-current-mapping-reconciliation.md`. No contract rule, no runtime code, and no mapping changed.
...
- **RECOMMENDATION_READY** (per Purpose): at least one PASS evidence item exists whose resulting `GoriyakuTag` is actually consumed by that Purpose's entry in the runtime mapping module `backend/temples/domain/need_to_goriyaku_tag_ids.py` (`NEED_TO_GORIYAKU_IDS`) — the single source of truth for Purpose routing, always read fresh, never duplicated here — or is matched by `NEED_TEXT_WEIGHTS`. **Note**: PASS alone does not guarantee this. Source-backed eligibility and Purpose wiring are independent axes (§3): wiring is never evidence, and evidence never implies wiring.
  - *Wired example (current):* `交通安全` (id 3), `航海安全` (id 13) and `海上安全` (id 14) **are** currently consumed by `travel_safe` (`travel_safe = {3, 13, 14}` in the runtime module, pinned by `backend/temples/tests/test_need_to_goriyaku_tag_ids.py`). A Source-backed PASS for one of these tags therefore *does* satisfy the Purpose-connectivity half of RECOMMENDATION_READY for `travel_safe` — it still requires a valid PASS under this contract (波上宮's voyage-safety prayer content is a real candidate for such a PASS).
  - *Unwired case (general):* some canonical `GoriyakuTag` concepts are consumed by no Purpose in the runtime module today (`docs/audit/shrine-evidence-integrity-full-audit.md` identifies 10). A correct PASS on one of those would not, by itself, make a shrine RECOMMENDATION_READY for any current Purpose. Whether such a concept should be wired is a separate Product decision — the **P7 `PURPOSE_MAPPING_REVIEW`** track — not decided by this contract or by a reviewer. This contract can only confirm Source-backing and correct canonical-tag selection; Purpose routing is governed entirely by the runtime module.
...
| 波上宮 | *(Batch 17 sources)* | Per `recommendation-evidence-followup-design.md` §6: multiple `source_confirmed` histories with explicit prayer/outcome language (豊漁・豊穣・平穏・航海の平安) | — | — | *(TBD — prior audit's finding: READY_FOR_RECOMMENDATION_EVIDENCE_REVIEW)* | `航海安全` / `海上安全` are currently consumed by `travel_safe` in the runtime module (§8), so a valid Source-backed PASS on 波上宮's voyage-safety content would contribute to RECOMMENDATION_READY for `travel_safe`. The PASS/HOLD decision itself is still not pre-decided by this template. |
...
- The `travel_safe` `NEED_TO_GORIYAKU_IDS` mapping-quality question flagged at PR #2572 §21 time (then `travel_safe = {10, 22, 23}`, none of which read as "travel safety" at the tag-name level) was **subsequently resolved**: a later PR corrected the mapping to `travel_safe = {3, 13, 14}` (交通安全 / 航海安全 / 海上安全), all direct name-level matches — see `docs/audit/goriyaku-mapping-master-integrity.md` and `docs/audit/goriyaku-mapping-master-integrity-correction.md`, pinned by `backend/temples/tests/test_need_to_goriyaku_tag_ids.py`. This contract still does not itself change any Purpose mapping; ids 13/14 are **no longer** an example of an unwired canonical tag (§8 updated accordingly).
- The earlier draft's note that several `NEED_TO_GORIYAKU_IDS` values referenced ids 42–45 (absent from the 39-row `GoriyakuTag` master) is also **stale**: those stale references were removed in the same correction PR and their non-recurrence is pinned by `test_ids_42_to_45_referenced_nowhere`. Purpose Mapping remains governed by the runtime module `backend/temples/domain/need_to_goriyaku_tag_ids.py`, not by any table duplicated here. Genuinely unwired canonical concepts that remain (`docs/audit/shrine-evidence-integrity-full-audit.md`: 10) are the subject of the separate **P7 `PURPOSE_MAPPING_REVIEW`** Product-decision track, not this contract.
