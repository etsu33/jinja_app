...
主テーマ（`history_theme`）と補助テーマ（`history_theme_secondary`）は性質が異なる。主テーマはRecommendation Reason・Meaning Card・Action Suggestion・Reflectionなど体験全体で参照される値である。補助テーマは現時点ではScore v3の内部シグナルとしてのみ利用され、画面表示・Reflection・Analyticsのいずれにも使用しない。

**「復興」は、現行の相談解釈では補助テーマとしてのみ生成され、主テーマとして生成されることはない。** 以下の対応表でも、「復興」は補助テーマの列にのみ記載する。

### 現行の相談解釈で認識される例

以下は、現行の相談解釈が実際に生成する対応の一部である。相談解釈が区別できる相談状態の種類は限定的であり、次項「意味的な近さを示す例」と混同しない。

| 相談状態の例 | 主テーマ | 補助テーマ | 解釈 |
|---|---|---|---|
| 不安が強い | 守り | 静寂 | 土台を整え、刺激から距離を置く |
| 疲れている | 静寂 | 復興 | 休息と回復を優先する |

正確な認識ロジック・対応する相談状態の種類・網羅範囲は、関連するBackend実装（相談解釈サービス）およびテストを正本とする。本書は実装のキー名・辞書内容を再掲しない。

### 意味的な近さを示す例（将来の拡張方向性）

以下は、相談内容と`history_theme`の意味的な関係を示す参考例であり、現行の相談解釈が機械的に生成する値を保証するものではない。実装を拡張する場合の方向性を示す目的でのみ扱う。

| 相談状態の例 | 意味的に近いテーマ | 意味的に関連するテーマ | 解釈 |
|---|---|---|---|
| 将来が見えない | 再出発 | 静寂 | 立ち止まり、方向を見直す |
| 落ち込んでいる | 静寂 | 復興 | エネルギーを取り戻す |
| やり直したい | 再出発 | 復興 | 区切りを作り、立て直す |
| 転職を考えている | 再出発 | 勝負 | 環境変化と決断 |
| 独立したい | 勝負 | 再出発 | 挑戦と新しい段階への移行 |
| 挑戦したい | 勝負 | 学び | 行動と準備 |
| 決断したい | 勝負 | 守り | 選択とリスク整理 |
| 自信がない | 学び | 復興 | 自己効力感と積み上げ |
...
この一覧で「復興」が現れる行は、いずれも「意味的に関連するテーマ」列に置く。「復興」を意味的に近いテーマ（主テーマ相当）として示す例はここには含めない。実装拡張により「復興」を主テーマとして生成する経路を追加する場合の判断は、Audit文書（`docs/audit/product-document-responsibility-audit.md`）の後続PR候補を参照する。
