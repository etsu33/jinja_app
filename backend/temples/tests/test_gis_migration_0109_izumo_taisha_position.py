"""Regression tests for temples.0109_adopt_izumo_taisha_position.
...
_mod = importlib.import_module("temples.migrations.0109_adopt_izumo_taisha_position")
PreconditionViolation = _mod.PreconditionViolation

PRE_0109 = [("temples", "0108_remove_legacy_temples_models")]
AT_0109 = [("temples", "0109_adopt_izumo_taisha_position")]

SHRINE_ID = 4
NAME = "出雲大社"
ADDRESS = "島根県出雲市大社町杵築東195"
OLD_LAT, OLD_LNG = 35.4016, 132.6853
NEW_LAT, NEW_LNG = 35.40190463, 132.68547534
...
def _historical_shrine_model(executor):
    state = executor.loader.project_state(PRE_0109[0])
...
def _migration_recorded():
    with connection.cursor() as cur:
        cur.execute(
            ...
            "WHERE app = 'temples' AND name = '0109_adopt_izumo_taisha_position'"
        )
...
def test_forward_updates_exact_audited_target(pre_0109):
    executor = pre_0109
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)

    _migrate(executor, AT_0109)
...
@pytest.mark.django_db(transaction=True)
def test_forward_does_not_read_or_write_legacy_text_location(pre_0109):
    executor = pre_0109
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
def test_forward_wrong_identity_fails_closed(pre_0109, name, address):
    executor = pre_0109
    Shrine = _historical_shrine_model(executor)
...
@pytest.mark.django_db(transaction=True)
def test_forward_unexpected_coordinate_fails_closed(pre_0109):
    executor = pre_0109
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine, lat=35.9999, lng=132.9999)
    before = _raw_row(SHRINE_ID)
...
@pytest.mark.django_db(transaction=True)
def test_forward_already_corrected_fails_closed(pre_0109):
    executor = pre_0109
    Shrine = _historical_shrine_model(executor)
...
@pytest.mark.django_db(transaction=True)
def test_absent_target_is_symmetric_noop(pre_0109):
    executor = pre_0109
...
@pytest.mark.django_db(transaction=True)
def test_reverse_restores_exact_old_coordinate(pre_0109):
    executor = pre_0109
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
@pytest.mark.django_db(transaction=True)
def test_reverse_unexpected_coordinate_fails_closed(pre_0109):
    executor = pre_0109
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
