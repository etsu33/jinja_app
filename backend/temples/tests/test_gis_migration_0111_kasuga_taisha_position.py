"""Regression tests for temples.0111_adopt_kasuga_taisha_position.
...
_mod = importlib.import_module("temples.migrations.0111_adopt_kasuga_taisha_position")
PreconditionViolation = _mod.PreconditionViolation

PRE_0111 = [("temples", "0110_adopt_fushimi_inari_position")]
AT_0111 = [("temples", "0111_adopt_kasuga_taisha_position")]

SHRINE_ID = 5
NAME = "春日大社"
ADDRESS = "奈良県奈良市春日野町160"
OLD_LAT, OLD_LNG = 34.6814, 135.8481
NEW_LAT, NEW_LNG = 34.6812901, 135.8482531

# 春日大社 is pk=5; 伏見稲荷大社 is pk=2 (the temples.0110 target, already at its
# adopted coordinate at PRE_0111) and must never be touched by 0111.
OTHER_SHRINE_ID = 2
OTHER_NAME = "伏見稲荷大社"
OTHER_ADDRESS = "京都府京都市伏見区深草薮之内町68"
OTHER_LAT, OTHER_LNG = 34.967133624329, 135.77318468005
...
def _historical_shrine_model(executor):
    state = executor.loader.project_state(PRE_0111[0])
...
def _migration_recorded():
    with connection.cursor() as cur:
        cur.execute(
            ...
            "WHERE app = 'temples' AND name = '0111_adopt_kasuga_taisha_position'"
        )
...
def test_forward_updates_exact_audited_target(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)

    _migrate(executor, AT_0111)
...
@pytest.mark.django_db(transaction=True)
def test_forward_does_not_read_or_write_legacy_text_location(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
def test_forward_wrong_identity_fails_closed(pre_0111, name, address):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
...
@pytest.mark.django_db(transaction=True)
def test_forward_unexpected_coordinate_fails_closed(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
...
@pytest.mark.django_db(transaction=True)
def test_forward_already_corrected_fails_closed(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
...
@pytest.mark.django_db(transaction=True)
def test_absent_target_is_symmetric_noop(pre_0111):
    executor = pre_0111
...
@pytest.mark.django_db(transaction=True)
def test_reverse_restores_exact_old_coordinate(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
@pytest.mark.django_db(transaction=True)
def test_forward_reverse_forward_is_deterministic(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
@pytest.mark.django_db(transaction=True)
def test_reverse_unexpected_coordinate_fails_closed(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
@pytest.mark.django_db(transaction=True)
def test_unrelated_shrine_is_untouched(pre_0111):
    executor = pre_0111
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
    _seed_other(Shrine)
