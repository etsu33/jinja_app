"""Regression tests for temples.0112_adopt_atsuta_jingu_position.
...
_mod = importlib.import_module("temples.migrations.0112_adopt_atsuta_jingu_position")
PreconditionViolation = _mod.PreconditionViolation

PRE_0112 = [("temples", "0111_adopt_kasuga_taisha_position")]
AT_0112 = [("temples", "0112_adopt_atsuta_jingu_position")]

SHRINE_ID = 7
NAME = "熱田神宮"
ADDRESS = "愛知県名古屋市熱田区神宮1-1-1"
OLD_LAT, OLD_LNG = 35.1279, 136.9114
NEW_LAT, NEW_LNG = 35.12737043, 136.90868002

# 熱田神宮 is pk=7; 春日大社 is pk=5 (the temples.0111 target, already at its
# adopted coordinate at PRE_0112) and must never be touched by 0112.
OTHER_SHRINE_ID = 5
OTHER_NAME = "春日大社"
OTHER_ADDRESS = "奈良県奈良市春日野町160"
OTHER_LAT, OTHER_LNG = 34.6812901, 135.8482531
...
def _historical_shrine_model(executor):
    state = executor.loader.project_state(PRE_0112[0])
...
def _migration_recorded():
    with connection.cursor() as cur:
        cur.execute(
            ...
            "WHERE app = 'temples' AND name = '0112_adopt_atsuta_jingu_position'"
        )
...
def test_forward_updates_exact_audited_target(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)

    _migrate(executor, AT_0112)
...
@pytest.mark.django_db(transaction=True)
def test_forward_does_not_read_or_write_legacy_text_location(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
def test_forward_wrong_identity_fails_closed(pre_0112, name, address):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
...
@pytest.mark.django_db(transaction=True)
def test_forward_unexpected_coordinate_fails_closed(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine, lat=35.9999, lng=136.9999)
    before = _raw_row(SHRINE_ID)
...
@pytest.mark.django_db(transaction=True)
def test_forward_already_corrected_fails_closed(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
...
@pytest.mark.django_db(transaction=True)
def test_absent_target_is_symmetric_noop(pre_0112):
    executor = pre_0112
...
@pytest.mark.django_db(transaction=True)
def test_reverse_restores_exact_old_coordinate(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
@pytest.mark.django_db(transaction=True)
def test_forward_reverse_forward_is_deterministic(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
@pytest.mark.django_db(transaction=True)
def test_reverse_unexpected_coordinate_fails_closed(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
...
@pytest.mark.django_db(transaction=True)
def test_unrelated_shrine_is_untouched(pre_0112):
    executor = pre_0112
    Shrine = _historical_shrine_model(executor)
    _seed_target(Shrine)
    _seed_other(Shrine)
