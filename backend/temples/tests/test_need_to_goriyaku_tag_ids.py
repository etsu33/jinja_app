...
def test_stale_ids_removed_from_previously_referencing_purposes():
    assert NEED_TO_GORIYAKU_IDS["relationship"] == {1, 27, 34}
    assert NEED_TO_GORIYAKU_IDS["health"] == {7, 8}
    assert NEED_TO_GORIYAKU_IDS["mental"] == {11, 16, 26, 28, 38}
    assert NEED_TO_GORIYAKU_IDS["rest"] == {7, 8}
    assert NEED_TO_GORIYAKU_IDS["family"] == {2, 25, 27, 34}


def test_purposes_outside_correction_scope_are_unchanged():
    # marriage/communication/courage/focus were out of scope for both the
    # original Option A correction and the master-integrity correction --
